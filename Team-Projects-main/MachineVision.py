...
def detect_line(debug, img, color):
    ...
    if debug:
        cv2.imshow("selected channel", selected_color)
        cv2.imshow("side1", side_color1)
        cv2.imshow("side2", side_color2)
    ...
    # gradations left
    selected_color = subtract_side_colors_and_equalize(selected_color, side_color1, side_color2)
    ...
    if debug:
        cv2.imshow("adap", selected_color)
    ...
    if debug:
        ...
        cv2.imshow("Bound Box", merged_image)

    # Find left or right quadrant for rotation and factor that into the value
    rectOuput = rect[2]

    if rect[0][1] > rect[1][1]:
        rectOuput = rectOuput - 90
